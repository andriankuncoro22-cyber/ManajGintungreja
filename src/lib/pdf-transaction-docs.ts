 ...
 * Sesuai format fisik:
 * - Halaman 1: NOTA PESANAN (Kop Surat Pemdes Gintungreja, Kepada Toko, Tabel Rincian, TTD Yang Menerima & Yang Memesan)
...
export const generateNotaDanFakturPDF = async (
  ...
  doc.setFontSize(13);
  doc.text("DESA GINTUNGREJA", (pageWidth / 2) + 8, 25, { align: "center" });

  doc.setFont("times", "normal");
  doc.setFontSize(9);
  doc.text("Sekretariat : Jl.Raya Gintungreja KM 05  Kode Pos 53254", (pageWidth / 2) + 8, 30, { align: "center" });
  ...
  doc.setFont("times", "normal");
  doc.setFontSize(10);
  doc.text(`Gintungreja, ${tanggalFormatIndo}`, 155, sign1DateY, { align: "center" });
  ...
  doc.text("Yth. Kepala Desa Gintungreja", 134, 31);
  doc.text("Di-", 143, 36);

  doc.setFont("times", "bold");
  doc.text("GINTUNGREJA", 148, 41);
  const wRk = doc.getTextWidth("GINTUNGREJA");
  doc.setLineWidth(0.3);
  ...
  doc.setFont("times", "normal");
  doc.setFontSize(10);
  doc.text(`Gintungreja, ${tanggalFormatIndo}`, 155, sign2DateY, { align: "center" });
...
export const generateStrukBelanjaPDF = async (data: StrukBelanjaData): Promise<Blob> => {
  ...
  doc.setFontSize(11);
  const tokoName = (data.namaToko || "MINIMARKET GINTUNGREJA JAYA").toUpperCase();
