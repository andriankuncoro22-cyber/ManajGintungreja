...
/**
 * Guard Komponen untuk memastikan hanya admin gintungreja@gmail.id 
...
function ManagementGuard({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!isUserLoading) {
      ...
      const isAllowed = user?.email?.toLowerCase() === "gintungreja@gmail.id";
...
export default function RootLayout({
  ...
  return (
    <html lang="id">
      <head>
        <title>Manajemen Desa Gintungreja</title>
        <meta name="description" content="Sistem Manajemen & Pelayanan Desa Gintungreja, Kec. Gandrungmangu, Kab. Cilacap" />
        ...
        <meta name="application-name" content="Gintungreja" />

        {/* ── iOS PWA (Add to Home Screen) ─────────────────────── */}
        <meta name="apple-mobile-web-app-capable" content="yes" />
        <meta name="apple-mobile-web-app-status-bar-style" content="black-translucent" />
        <meta name="apple-mobile-web-app-title" content="Gintungreja" />
                  ...
                  <div className="flex min-h-screen w-full overflow-hidden">
                    ...
                    <SidebarInset className="flex-1 flex flex-col min-w-0">
                      <header className="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-2 border-b bg-background px-4 md:hidden">
                        ...
                        <div className="flex-1 text-center font-black text-primary uppercase tracking-tighter">Gintungreja</div>
